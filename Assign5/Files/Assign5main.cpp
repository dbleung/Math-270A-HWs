...
int main()
{
	...
	GridFun1D dtTemp(jstar - 1, xMin, xMax,values1d);//for variable time stepping, keeps values of dt for forward and backward sweep
	...
	//cout << "dtTemp" << endl;
	//print_vec(dtTemp.values, jstar - 1);
	...
	//cout << " uExact is:" << endl;
	//print_array(uExact, uExact.xPanel, uExact.yPanel);

	//cout << " f is:" << endl;
	//print_array(f, f.xPanel, f.yPanel);

	//cout << " uk is:" << endl;
	//print_array(uk, uk.xPanel, uk.yPanel);
	...
	while((diffNorm > tol)&&(iter < iterMax))
	{
		...
		// Go through forward sweep (NEED TO RUN THIS OVER WITH MAX PANEL SIDE?)

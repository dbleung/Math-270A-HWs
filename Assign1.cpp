...
public:
	...
	TriSolver(const TriSolver& T) // called when you declare an instance with an existing instance, & is pass be ref, memory
	{
		initialize(T);
	};
	...
	*/
	...
	void initialize(const TriSolver& T) // note: & is pass by reference, but putting a class in a variable declaration??
	{
		systemSize = T.systemSize;
		loDiag = T.loDiag;
		diag = T.diag;
	...
	*/
    ...
    void apply(vector<double>& f, vector<double>& u) //Don't I need to input loDiag, Diag, etc.? Why f and u?
    {
